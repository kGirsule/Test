 ...
 * Sie regelt alles, was nur der Ritter können sollte
 *
 * @author Katharina Girsule
 * @version 31.5.2020
 */
public class Knight extends Spielerklassen {

    /**
     * Subklasse Magier, implementiert die speziellen Fähigkeiten des Ritters
     ...
     */
    public Knight(Field field, Position position, int hp, int movement, Ability ability1, Ability ability2, Ability ability3) {
    ...
    public String descriptionAb1(){
        return super.descriptionAb1()+"\t"+" You can attack a two Targets in range";
    }
    ...
    public String descriptionAb2(){
        return super.descriptionAb2()+"\t"+" Every Enemy next to you has to attack you";
    }
    ...
    public String descriptionAb3(){
        return super.descriptionAb3()+"\t"+" Every ally in range gains a chance to avoid damage when they get attacked";
    }
    ...
    protected void attackingAb1(Position spot)  {
        ...
        Position nextSpot = newPosition(givePosition(), getAbility1().getRange());

        if (giveField().giveObjektAt(nextSpot) != null) {
            Akteur a = (Akteur) giveField().giveObjektAt(spot);
            a.takesDamage(getDamage());
        }
    }
    ...
    protected void attackingAb3(Position spot){
        ArrayList<Akteur> avoidDamage = giveField().enemysInRange(spot, 2);
        for (Akteur a : avoidDamage) {
            if(a instanceof Spielerklassen){
                a.setAvoiding(true);
            }
        }
